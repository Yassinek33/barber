        ...
        <Reveal className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-semibold mb-2">
              ...
              <span>Echte Klantbeoordelingen Groningen</span>
            </div>
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-white tracking-tight">
              Wat onze <span className="gold-text-gradient">Gentlemen</span> zeggen
            </h2>
            <p className="text-slate-400 text-xs sm:text-sm mt-1">
              Gemiddelde score van 4.9/5★ op basis van meer dan 488 geverifieerde reviews.
            </p>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {rev.verified && (
                          <CheckCircle className="w-3.5 h-3.5 text-emerald-400" title="Geverifieerde Klant" />
                        )}
                      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="relative w-full max-w-lg bg-slate-900 border border-amber-500/30 rounded-2xl p-6 shadow-2xl">
            ...
            <h3 className="font-display text-xl font-bold text-white mb-4">Geef uw Klantbeoordeling</h3>

            <form onSubmit={handleAddReview} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Uw Naam / Bijnaam:</label>
                <input
                  type="text"
                  required
                  placeholder="bijv: Julien D."
                  value={newAuthor}
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Tevredenheidsscore:</label>
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Uitgevoerde Dienst:</label>
                <select
                  value={newService}
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Uw Opmerking:</label>
                <textarea
                  required
                  rows={3}
                  placeholder="Deel uw ervaring met de knipbeurt, de sfeer en de barbier..."
                  value={newText}
